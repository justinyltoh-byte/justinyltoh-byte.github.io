...
df <- read_excel("C:/Users/justi/Downloads/My reinterpreted results/2017 Christiano transcriptomic and proteomic/2017_RNAseq_SuppS10.xlsx")
...
ggplot(df, aes(x = `Log2 FC MF/PF`, y = `-Log10(P-Value)`, color = significance)) +
  ...
  annotate("text", x = 3.25, y = 6.5, label = paste0("Up: ", up_count), color = "red", fontface = "bold") +
  ...
  labs(title = "Transcriptomic Differential Expression: MF vs PF",
